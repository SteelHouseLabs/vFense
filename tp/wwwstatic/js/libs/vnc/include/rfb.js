...
function constructor() {
    ...
    if (Websock_native) {
        ...
        updateState('loaded', 'Ready: native WebSockets, ' + rmode);
    } else {
        ...
        if ((! Util.Flash) ||
            ...
            updateState('loaded', 'Ready: WebSockets emulation, ' + rmode);
        }
    }
...
function connect() {
    ...
    if (typeof UsingSocketIO !== "undefined") {
        ...
        uri = "https://" + rfb_host + "/" + rfb_path + "/" + rfb_hostname + "/" +rfb_port;
    } else {
        if (conf.encrypt) {
            uri = "wss://";
        } else {
        ...
        uri += rfb_host + "/" + rfb_path + "/" + rfb_hostname + "/" +rfb_port;//rfb_host + ":" + rfb_port + "/" + rfb_path;
    }
...
that.connect = function(host, port, password, path, hostname) {
    //Util.Debug(">> connect");

    rfb_host       = host;
    rfb_port       = port;
    rfb_hostname   = hostname;
